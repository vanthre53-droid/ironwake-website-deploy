...
notification state base is live at `354c9cb`; W15 worker execution, W16
follow-up, W14 owner MFA/complete operations, and W22 reproducible deployment
remain missing, failed live, or implemented-only. No row is release-green. M1
owns the next local notification worker task.

Resend Free is selected for code only at C1 checkpoint `27db52d`.
`IW-P0-NOTIFY-01` is verified live at `354c9cb`; M1 now owns
`IW-P0-NOTIFY-02`, the fail-closed local adapter/worker task. Account terms,
domain/DNS, secrets, sends, webhook registration, and deployment remain G4/G5
human-controlled gates and do not block local code and no-network tests.
...
| W14 | Notifications/providers | partial_state_machine_live_provider_pending | Codex M1 + provider/human; C2 audit | `reports/REAL_CAPABILITY_LEDGER.md`, `reports/evidence/P2_SCHEMA_AND_RLS.md`, `reports/evidence/P4_PROVIDER_PROOF.md` | Notification state is live; implement the fail-closed worker locally, then keep connection/send proof behind G4/G5 |

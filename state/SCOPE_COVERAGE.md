...
`42c5754`; W15 signed callbacks/provider execution, W16 follow-up, W14 owner
MFA/complete operations, and W22 reproducible deployment remain missing, failed
live, or implemented-only. No row is release-green. M1 owns the next local
signed-webhook task.

Resend Free is selected for code only at C1 checkpoint `27db52d`.
`IW-P0-NOTIFY-01` is verified live at `354c9cb` and `IW-P0-NOTIFY-02` is
verified local at `42c5754`. M1 now owns `IW-P0-NOTIFY-03`, the signed-webhook
route and fixture-test task. Account terms, domain/DNS, secrets, sends, webhook
registration, and deployment remain G4/G5 human-controlled gates and do not
block local code and no-network tests.
...
| W14 | Notifications/providers | partial_worker_local_provider_pending | Codex M1 + provider/human; C2 audit | `reports/REAL_CAPABILITY_LEDGER.md`, `reports/evidence/P2_SCHEMA_AND_RLS.md` | State is live and worker is local-only; implement the signed webhook locally, then keep registration/send proof behind G4/G5 |

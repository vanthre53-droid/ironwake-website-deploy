...
// ponytail: Google OAuth server action used by /login and /signup
// "Continue with Google" controls. The server action calls Supabase
// signInWithOAuth, which returns a one-time redirect URL bound to our
// allowlisted production callback. We never expose any Supabase key to
// the browser. The provider URL is then followed via Next's `redirect`.
// redirectTo here is the *post-callback* destination; the actual OAuth
// callback is always /auth/callback and `next` is forwarded through it.
export async function signInWithGoogleAction(redirectTo = '/account') {
  const supabase = await createServerSupabase();
  if (!supabase) return { error: 'Sign-in is not connected yet. Please try again shortly.' };
  const safeNext = safeAuthRedirect(redirectTo || '/account');
  const callbackUrl = `https://ironwake.dev/auth/callback?next=${encodeURIComponent(safeNext === 'https://ironwake.dev' ? '/account' : safeNext.replace('https://ironwake.dev', '') || '/account')}`;
  ...
  return { error: 'Google sign-in is not configured for this site yet.' };
}

function siteOrigin() {
  // ponytail: safeAuthRedirect is the single source of truth for both
  // origin validation and canonical fallback (see lib/auth-redirect-allowlist.mjs).
  return safeAuthRedirect(process.env.NEXT_PUBLIC_SITE_URL || '/');
}

function emailRedirect(path) {
  // ponytail: paths here come from server actions we author (signup,
  // forgot-password, etc). safeAuthRedirect still re-validates against
  // the allowlist so a caller cannot smuggle an off-origin path.
  const safe = safeAuthRedirect(path);
  return safe.startsWith('https://ironwake.dev') ? safe : `${siteOrigin()}${path.startsWith('/') ? path : `/${path}`}`;
}

...
// ponytail: replaced `node:crypto.randomBytes` with `crypto.getRandomValues`
// so this server-action module stays edge-runtime safe. The OAUTH state is
// only used as an opaque CSRF nonce; 24 bytes of entropy is unchanged.
// See meta-webhook-verify.mjs for the same Worker-bundle-shrink rationale.
const oauthState = () => {

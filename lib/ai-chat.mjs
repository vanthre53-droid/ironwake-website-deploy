...
function parseProviderOutput(value) {
  ...
  // defaults so the visitor still gets a real answer. Strip any chain-of-
  // thought block the provider leaks into content, then keep the first 600
  // chars (matching the JSON envelope length bound).
  let text = value.trim();
  ...
  const hasContact = /(contact|book|audit|form|\/book|\/audit)/.test(lower);
  return {
    reply: text,
...
function knowledgeBlock() {
  ...
  return [
    'IronWake is a commercial systems company that designs, builds, and deploys custom websites, AI receptionists, booking systems, CRM/follow-up automation, and lead-recovery systems for service businesses. IronWake owns the implementation work it sells — design, frontend and backend code, deployment, and ongoing support — rather than outsourcing it.',
    'IronWake sells five commercial offers (Business Leak Audit, Conversion Website, AI Receptionist Deployment, CRM & Follow-up Automation, Booking & Scheduling). The four implementation offers are real productized custom-build engagements delivered by IronWake; the Business Leak Audit is the optional paid diagnostic that precedes a custom engagement:',
    offerLines,
    'Conversion Website, AI Receptionist Deployment, CRM & Follow-up Automation, and Booking & Scheduling are scoped, estimated, and delivered by IronWake. The Business Leak Audit is the optional paid diagnostic that identifies which of those offers fits a specific business — it is the entry point, not the whole company. Tier and final pricing for any engagement varies by scope, geography, and integrations; refer visitors to /pricing for the published tiers or to /audit to start a scope conversation.',
  ...
  try {
    const response = await fetchImpl(endpoint, {
      ...
      body: JSON.stringify({
        ...
        // ponytail: reasoning_split=true stalls the provider past the 12s
        // worker timeout on this 3.7KB system prompt. Disabled; think-chain
        // content is stripped by parseProviderOutput regardless.

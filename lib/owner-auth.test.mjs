...
test('auth errors retain safe operational classes', () => {
  ...
  assert.equal(classifyAuthError({ code: 'invalid_grant' }), 'session_expired');
  assert.equal(classifyAuthError({ message: 'both auth code and code verifier should be non-empty' }), 'session_expired');
});

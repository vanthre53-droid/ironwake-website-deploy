...
// ponytail: replaced `node:crypto.randomUUID` with the Web Crypto API's
// `crypto.randomUUID()` so this module can be bundled into the edge
// runtime. CF Workers + Next.js edge runtime both expose
// `crypto.randomUUID()` since the Workers runtime 18+ and Next 14.
...
export async function runCronInvocation({
  ...
  presentedToken = '',
  workerId = `ironwake-notification-${uuid()}`,

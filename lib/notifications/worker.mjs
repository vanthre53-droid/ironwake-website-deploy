// ponytail: replaced `node:crypto.randomUUID` with the Web Crypto
// `crypto.randomUUID()` so this module bundles into the edge runtime.
// See cron-handler.mjs for the same rationale (Worker bundle size).
...
export async function runNotificationWorker({
  ...
  workerId = `ironwake-notification-${crypto.randomUUID()}`,
  limit = 10,
  inquiryId = null,
  eventId = null
} = {}) {

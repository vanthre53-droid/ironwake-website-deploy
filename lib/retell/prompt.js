...
//   "too expensive"  → point to the Audit (Lite ₹799 / $29) as the cheapest
//                       entry point and the dual-region price schedule; do
//                       not invent discounts.
...
export function buildReceptionistPrompt({ language = 'en-IN', knowledge } = {}) {
  ...
  return [
    ...
    '- CONSIDERING: lead comparing providers — anchor on the differentiator (every engagement starts with a Business Leak Audit; India + International price schedules; no automated payment on site; /work is capability proof, not client outcomes).',

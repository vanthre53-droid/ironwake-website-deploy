...
// ponytail: portfolio demonstration — designed scenario for Sameerscissor.com (Delhi), not a real engagement.
...
const steps = [
  {
    ...
    text:
      'A caller wants a booking for Sameerscissor.com (Delhi). Staff cant pick up. Without capture, the caller phones the next salon on Google.',
  },
  {
    num: '02',
    title: 'AI answers with the salons own brand voice',
    text:
      'The voice agent picks up, introduces itself as Sameerscissor.com (Delhi)'s assistant, and asks which service and preferred time.',
  },
  {
    num: '03',
  ...
  },
  {
    ...
    text:
      'A reminder 24h before. After-hours calls still get answered. Every opportunity stays in Sameerscissor.com (Delhi)'s pipeline.',
  },
];

const features = [
  ['Answers every call', 'Voice AI picks up on the first ring, in Sameerscissor.com (Delhi)'s brand voice, even when the team is busy.'],
...
export function SameerscissorComDelhiInCaseStudy() {
  return (
    <CaseStudyStory
      kicker="DEMONSTRATION"
      title="{title}"
      subtitle="A personalised voice-AI booking demo for Sameerscissor.com (Delhi) in Delhi, IN."
      heroNote="Built with only public business info (name, address, phone, services). Not a client engagement. Not a measured outcome."
      steps={steps}
      features={features}
      accent="#0a0a0a"
      accentSoft="#f5f1ea"
      ctaLabel="See the live demo for Sameerscissor.com (Delhi)"
      ctaHref="/work/sameerscissor-com-delhi-in/demo.html"
    />
  );
}

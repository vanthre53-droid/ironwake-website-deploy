...
// ponytail: portfolio demonstration — designed scenario for Above Salons (Leeds), not a real engagement.
...
const steps = [
  {
    ...
    text:
      'A caller wants a booking for Above Salons (Leeds). Staff cant pick up. Without capture, the caller phones the next salon on Google.',
  },
  {
    num: '02',
    title: 'AI answers with the salons own brand voice',
    text:
      'The voice agent picks up, introduces itself as Above Salons (Leeds)\'s assistant, and asks which service and preferred time.',
  },
  {
    num: '03',
  ...
  },
  {
    ...
    text:
      'A reminder 24h before. After-hours calls still get answered. Every opportunity stays in Above Salons (Leeds)\'s pipeline.',
  },
];

const features = [
  ['Answers every call', 'Voice AI picks up on the first ring, in Above Salons (Leeds)\'s brand voice, even when the team is busy.'],
  ...
  ['Speaks Leeds local', 'Trained on UK English / Indian English accents as appropriate for Leeds.'],
...
export function AboveSalonsLeedsUkCaseStudy() {
  return (
    <CaseStudyStory
      kicker="DEMONSTRATION"
      title="{title}"
      subtitle="A personalised voice-AI booking demo for Above Salons (Leeds) in Leeds, UK."
      heroNote="Built with only public business info (name, address, phone, services). Not a client engagement. Not a measured outcome."
      steps={steps}
      features={features}
      accent="#0a0a0a"
      accentSoft="#f5f1ea"
      ctaLabel="See the live demo for Above Salons (Leeds)"
      ctaHref="/work/above-salons-leeds-uk/demo.html"
    />
  );
}

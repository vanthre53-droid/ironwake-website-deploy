...
export function OwnerDashboard() {
  ...
  return <main className="shell owner-shell">
    <section className={`owner-card${authorization.allowed ? ' crm-owner-card' : ''}`}>
      <span className="eyebrow">{authorization.allowed ? 'Private / operations' : 'Private / owner only'}</span>
      {!authorization.allowed && <h1>Owner CRM</h1>}
      {!session ? <>
        ...
        {mfa.status !== 'idle' && mfa.status !== 'error' ? <><p>This owner session is authenticated but private CRM access requires AAL2.</p>{mfa.status === 'enroll_pending' && <section className="crm-detail-note"><h2>Set up authenticator MFA</h2><p>Scan this QR code with your authenticator app, then enter the six-digit code.</p>{mfa.qrCode && <img src={mfa.qrCode} alt="TOTP enrollment QR code" width={192} height={192} className="mfa-qr" />}<p className="micro">Manual setup secret: <code>{mfa.secret}</code></p></section>}<form className="owner-form" onSubmit={verifyMfa}><label>Authenticator code<input inputMode="numeric" pattern="[0-9]{6}" name="mfaCode" value={mfaCode} onChange={(event) => setMfaCode(event.target.value)} autoComplete="one-time-code" required /></label><button className="button" type="submit">Verify MFA</button></form>{mfa.reason && <p className="notice" role="status">{mfa.reason}</p>}</> : <><p>This account is not the authorized owner for the IronWake CRM.</p><p className="notice" role="status">{mfa.reason || authorization.reason || 'Sign in with the designated owner email to continue.'}</p></>}
        <button className="button" onClick={signOut}>Sign out</button>
      </> : <>
        <header className="crm-heading">
          <div><span className="eyebrow">Owner workspace</span><h1>CRM control room</h1><p>Review real inquiries, keep the next action visible, and leave an auditable owner note.</p></div>
          <nav className="crm-heading-actions" aria-label="Owner workspace actions"><a className="button secondary" href="/admin">Notification status</a><button type="button" className="button secondary" onClick={signOut}>Sign out</button></nav>
        </header>
        <p className="crm-session-note">Authenticated owner session · private records remain protected by Supabase policy and AAL2.</p>
        <div className="crm-summary" aria-label="CRM summary"><div><span>Visible inquiries</span><strong>{visibleInquiries.length}</strong></div><div><span>Open follow-up</span><strong>{tasks.length}</strong></div><div><span>Overdue</span><strong className={overdueTasks.length ? 'metric-alert' : ''}>{overdueTasks.length}</strong></div></div>
        ...
        <div className="crm-workspace"><ul className="record-list" aria-label="Recent inquiries">
          {visibleInquiries.length ? visibleInquiries.map((inquiry) => <li key={inquiry.id}><button type="button" className={`record-card${selected?.id === inquiry.id ? ' selected' : ''}`} onClick={() => setSelectedId(inquiry.id)}>
            <div className="record-card-top"><span className="status-pill">{inquiry.lead_stage}</span><span className="record-date">{formatDue(inquiry.created_at)}</span></div><h3>{inquiry.business_name}</h3>
            <dl>

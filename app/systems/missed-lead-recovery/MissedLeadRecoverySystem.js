...
export function MissedLeadRecoverySystem() {
  ...
  return <main className="shell">
    ...
    <section className="section intro"><span className="eyebrow">Pick a way a lead usually gets lost</span><h2>See how the system responds.</h2><div role="group" aria-label="Lead-loss scenarios" className="stage-filter">{scenarios.map(([id, label]) => <button type="button" key={id} aria-pressed={active === id} onClick={() => setActive(id)}>{label}</button>)}</div><div className="disclosure-box" role="status">{current[2]}</div></section>
    <section className="journey"><span className="eyebrow">Workflow</span><h2>What actually happens, in order.</h2><div className="journey-grid">{steps.map(([label, title, text]) => <article key={title}><span className="micro">{label}</span><h3>{title}</h3><p>{text}</p></article>)}</div></section>
    <section className="section disclosure"><div><span className="eyebrow">Proof status</span><h3>What is verified today versus still pending.</h3><p>This page does not claim a finished product. It states what is implemented and tested against what remains in progress.</p></div><div className="disclosure-box"><p><span className="status-pill">Verified locally</span> Durable inquiry record, consent capture, and task assignment are implemented and tested.</p><p><span className="status-pill">Provider proof pending</span> Outbound email and WhatsApp delivery exist as a queued, retried attempt; production-scale delivery monitoring is not yet complete. No price or fixed response-time commitment is attached to this system.</p></div></section>
    <section className="section"><span className="eyebrow">Next step</span><h2>See it applied to your own inquiry path.</h2><a className="button" href="/audit">Request a Business Leak Audit</a></section>
    <SiteFooter />
  </main>;
}

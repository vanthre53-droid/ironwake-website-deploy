...
export function AiReceptionistSystem() {
  ...
  return <main className="shell">
    ...
    <section className="section intro"><span className="eyebrow">Operating principles</span><h2>What any future version would commit to.</h2><div className="system-grid">{principles.map(([label, title, text]) => <article className="system-card" key={title}><span className="micro">{label}</span><h3>{title}</h3><p>{text}</p></article>)}</div></section>
    <section className="section"><span className="eyebrow">Illustrative only</span><h2>What a disclosed first response could sound like.</h2><button type="button" className="button secondary" aria-expanded={showSample} onClick={() => setShowSample(!showSample)}>{showSample ? 'Hide sample transcript' : 'Show sample transcript'}</button>{showSample && <div className="disclosure-box" role="region" aria-label="Illustrative sample transcript"><p><strong>ILLUSTRATIVE SAMPLE — not a real call. No live phone line is connected.</strong></p>{transcript.map(([speaker, line], index) => <p key={index}><span className="micro">{speaker}</span> {line}</p>)}</div>}</section>
    <section className="section disclosure"><div><span className="eyebrow">What is real today</span><h3>No live call, no live pricing, no connected number.</h3><p>This page describes a concept, not a shipped product.</p></div><div className="disclosure-box">No phone number is connected to this concept page. No AI voice or chat model handles a live call or conversation for IronWake or any client through this page. A future pilot would need explicit approval, a disclosed recording and transcription policy, and a verified human escalation path before a single real conversation is handled.</div></section>
    <section className="section"><span className="eyebrow">Next step</span><h2>Tell us about your reception gap.</h2><a className="button" href="/audit">Talk to us about this concept</a></section>
    <SiteFooter />
  </main>;
}

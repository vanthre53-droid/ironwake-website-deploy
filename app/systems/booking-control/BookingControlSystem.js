...
export function BookingControlSystem() {
  ...
  return <main className="shell">
    ...
    <section className="section intro"><span className="eyebrow">Step through the states</span><h2>Only a real confirmation reaches Confirmed.</h2><div role="group" aria-label="Booking states" className="stage-filter">{states.map(([id, label]) => <button type="button" key={id} aria-pressed={active === id} onClick={() => setActive(id)}>{label}</button>)}</div><div className="disclosure-box" role="status">{current[2]}</div></section>
    <section className="journey"><span className="eyebrow">After a confirmation</span><h2>What a confirmed slot can become.</h2><div className="journey-grid">{after.map(([label, title, text]) => <article key={title}><span className="micro">{label}</span><h3>{title}</h3><p>{text}</p></article>)}</div></section>
    <section className="section disclosure"><div><span className="eyebrow">Proof status</span><h3>Request-only until a provider is verified.</h3><p>Until a calendar provider is connected and verified, every booking on this site stays a reviewed request rather than a confirmed slot.</p></div><div className="disclosure-box"><span className="status-pill">Request-only today</span> See the same honesty standard applied on the live <a href="/book">booking request flow →</a>. No price or availability guarantee is attached to this system.</div></section>
    <section className="section"><span className="eyebrow">Next step</span><h2>Review your own booking handoff.</h2><a className="button" href="/audit">Request a Business Leak Audit</a></section>
    <SiteFooter />
  </main>;
}

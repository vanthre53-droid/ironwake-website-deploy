...
export default function IndustriesPage() {
  return <main className="shell">
    ...
    <section className="section intro"><span className="eyebrow">Choose your industry</span><h2>Start with the workflow you recognise.</h2><div className="industry-grid">{industries.map(([href, label, title, text]) => <a href={href} key={href}><article className="industry-card"><span className="micro">{label}</span><h3>{title}</h3><p>{text}</p><span className="card-link">View industry guide →</span></article></a>)}</div></section>

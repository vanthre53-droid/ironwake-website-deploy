-- Allows a service-role controlled test or repair to claim one exact outbox event.
-- The optional selector does not alter ordinary scheduled claim behaviour.
drop function if exists public.claim_notification_events(text, text, integer, uuid);
